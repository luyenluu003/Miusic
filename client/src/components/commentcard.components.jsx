...
import CommentField from "./comment-field.components"
...
const CommentCard = ({index , leftVal,commentData}) =>{
    ...
    const LoadMoreRepliesButton = () => {
        ...
        let button = (
            <button onClick={() => loadReplies({ skip: index - parentIndex, currentIndex: parentIndex })} className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-sm flex items-center gap-2">
                Load more replies
            </button>
        );
        ...
        if (parentIndex !== undefined && commentsArr[parentIndex]) {
            if (commentsArr[index + 1]) {
                ...
                if (commentsArr[parentIndex].children) {
                    if ((index - parentIndex) < commentsArr[parentIndex].children.length) {
                        return button;
                    }
                }
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <div className="flex gap-5 items-center mt-5">

                    {
                        ...
                        <button className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md felx items-center gap-2" onClick={hideReplies}>
                            <i className="fi fi-rs-comment-dots"></i> Hide Reply
                        </button> :
                        <button className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md felx items-center gap-2" onClick={loadReplies}>
