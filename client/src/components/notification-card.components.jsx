...
const NotificationCard = ({data,index,notificationState}) => {
    ...
    return (
        <div className={"p-6 border-b border-grey border-1-black " + ( !seen ? "border-l-2": '')}>
            ...
            {
                type != 'like' ?
                <p className="ml-14 pl-5 font-gelasio text-xl text-dark-grey my-5">
                    {comment.comment}
                </p> : ""
            }

            <div className="ml-14 pl-5 mt-3 text-white/85 flex gap-8">
                <p>{getDay(createdAt)}</p>
                {
                    ...
                    <>
                        {
                            !reply ? 
                            <button className="underline hover:text-blue-gwen" onClick={handleReplyClick}>Reply</button>
                            :""

                        }
                        <button className="underline hover:text-blue-gwen" onClick={(e)=> handleDelete(comment._id,"comment",e.target)}>Delete</button>
                    </> :""
                }
            </div>
            {
...

            {
                ...
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                    ...
                    <button className="underline hover:text-blue-gwen ml-14 mt-2" onClick={(e)=> handleDelete(comment._id,"reply",e.target)}>Delete</button>
